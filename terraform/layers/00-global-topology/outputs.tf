...
output "vault_pki" {
  ...
  sensitive   = true
  value = {
    ca_cert        = tls_self_signed_cert.root_ca.cert_pem
    server_cert    = tls_locally_signed_cert.vault_server.cert_pem
    server_key     = tls_private_key.vault_server.private_key_pem
    haproxy_bundle = "${tls_locally_signed_cert.vault_server.cert_pem}\n${tls_private_key.vault_server.private_key_pem}"
  }
}
