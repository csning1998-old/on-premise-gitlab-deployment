...
locals {
  ...
  component_roles = flatten([
    for key, item in local._flat_catalog : {
      ...
      # DNS SAN Strategy
      # Case A: Entrypoint matches service name -> gitlab.production.iac.local
      # Case B: Entrypoint is a sub-component -> kas.gitlab.production.iac.local
      dns_san = flatten([
        for i_key, i_data in coalesce(item.config.ingress, {}) : [
          for sub in i_data.subdomains :
          sub == item.service_name ?
          "${sub}.${item.stage}.${var.domain_suffix}" :                    # Case A
          "${sub}.${item.service_name}.${item.stage}.${var.domain_suffix}" # Case B
        ]
      ])
