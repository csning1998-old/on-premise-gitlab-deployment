...
locals {
  ...
  vault_role_name = data.terraform_remote_state.vault_core.outputs.pki_configuration.ingress_roles["${local.platform_id}-ingress"].name
  domain_list     = data.terraform_remote_state.vault_core.outputs.pki_configuration.ingress_roles["${local.platform_id}-ingress"].allowed_domains
}
