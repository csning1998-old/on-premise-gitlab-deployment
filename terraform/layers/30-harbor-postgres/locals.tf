...
locals {
  ...
  # harbor-postgres falls under `${ProjectCode}-${Service}-${Component}` -> `harbor-postgres`
...
locals {
  ...
  # Key: "${service}-${dependency}-dep" -> "harbor-postgres-dep"
...
locals {
  ...
  flat_node_map = merge([
    for comp_name, comp_data in var.harbor_postgres_config : {
