  ...
  select layer in "${layer_options[@]}"; do
    if [[ "$layer" == "Back to Main Menu" ]]; then
      ...
      if [[ "$layer" == "10-vault-core" ]]; then
				...
				terraform_layer_executor "${layer}" "-target=module.vault_pki_setup -target=module.vault_workload_identity_ingress -target=module.vault_workload_identity_databases -refresh=false"
